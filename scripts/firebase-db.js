...
function sanitizeStatus(status) {
    if (!status) return "Your package is been updated";
    const lower = status.toLowerCase();
    if (lower === 'on hold' || lower === 'customs hold' || lower.includes('hold')) {
        return "Your package is been updated";
    }
...
function sanitizeHistoryItem(item) {
    ...
    if (item.description) {
        ...
        // Replace automated routing mentions with updated route
        desc = desc.replace(/automated routing update\.?/gi, "Updated route update: Your package is been updated.")
                   .replace(/automated routing/gi, "updated route")
                   .replace(/automated route/gi, "updated route")
                   .replace(/automated/gi, "updated");

        // Replace on hold mentions
        desc = desc.replace(/your package is on hold\.?/gi, "Your package is been updated.")
                   .replace(/package is on hold\.?/gi, "Your package is been updated.")
                   .replace(/on hold\.?/gi, "been updated.");
        ...
        item.description = "Your package is been updated.";
        modified = true;
    }
...
async function runDatabaseMigration() {
    try {
        ...
        if (firestoreDb) {
            // Migrate shipments with 'On Hold' or legacy routing status
            ...
            if (batchCount > 0) {
                await batch.commit();
                console.log(`Migrated ${batchCount} legacy records in Firestore to 'Updated Route' / 'Your package is been updated'.`);
            }
        }
    } catch (e) {
