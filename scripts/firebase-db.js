...
function sanitizeStatus(status) {
    ...
    if (lower === 'your package is been updated' || lower === 'package is been updated') {
        return "On Hold";
    }
...
function sanitizeHistoryItem(item) {
    ...
    if (item.description) {
        ...
        desc = desc.replace(/automated routing update\.?/gi, "Updated route update.")
                   .replace(/automated routing/gi, "updated route")
                   .replace(/automated route/gi, "updated route")
                   .replace(/automated/gi, "updated");

        // Replace been updated mentions with on hold
        desc = desc.replace(/your package is been updated\.?/gi, "Your package is on hold.")
                   .replace(/package is been updated\.?/gi, "Your package is on hold.")
                   .replace(/been updated\.?/gi, "on hold.");
        ...
        item.description = "Your package is on hold.";
        modified = true;
    }
...
async function runDatabaseMigration() {
    try {
        ...
        if (firestoreDb) {
            ...
            if (batchCount > 0) {
                await batch.commit();
                console.log(`Migrated ${batchCount} legacy records in Firestore to 'On Hold' / 'Updated Route'.`);
            }
        }
    } catch (e) {
        ...
        for (let i = 0; i < intervalsPassed; i++) {
            if (newIndex < routes.length - 1) {
                ...
                historyUpdates.push({
                    ...
                    status: status,
                    description: "Updated route update.",
